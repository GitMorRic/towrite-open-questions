...
| 按键 | 单击 | 双击 | 长按 |
| --- | --- | --- | --- |
| 左键 | 上一页面 | 上一任务卡 | 开始、暂停或继续计时 |
| 主键 | 开始并打开当前笔记 | 把冻结卡片发送到手机 PWA | 预留录音 |
...
仓库内的 `towrite-panel-driver.h` 默认采用失败关闭：它不会把串口输出伪装成
物理刷新成功。先识别屏幕控制器、载板和引脚，再实现这个硬件边界。在
`begin()` 与渲染方法能返回真实控制器结果前，固件不会发送 display ACK，
也不会允许按键事件。
...
`renderCard()`。串口只用于诊断，默认驱动模板返回 `false`；只有物理屏幕
控制器确认刷新完成后才能返回 `true`。
