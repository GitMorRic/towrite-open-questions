...
<section class="work-pool" aria-label="统一工作池">
  <header class="pool-toolbar">
    <div>
      ...
      <small>任务、问题和笔记仍写回各自 Markdown、Sidecar 与 frontmatter。</small>
    </div>
    <div class="toolbar-actions">
      {#if dailyApi.syncMarkdownTasks}
        <button type="button" disabled={Boolean(busy)} title="后台识别 Vault 中已有的 - [ ] 待办" on:click={syncMarkdownTasks}><ListTodo size={14} />扫描 Markdown 待办</button>
      {/if}
      {#if dailyApi.openTaskPoolSource}
        <button type="button" on:click={() => dailyApi.openTaskPoolSource?.()}><BookOpen size={14} />编辑任务 Markdown</button>
      {/if}
      <button type="button" title="刷新" aria-label="刷新工作池" on:click={() => reload()}><RefreshCw size={15} /></button>
    </div>
  </header>
  ...
  <details class="quick-create">
    ...
    <form on:submit|preventDefault={createTask}>
      <input class="task-input" bind:value={draftText} placeholder="准备推进什么？" required />
                  ...
                  <details class="subgroup" open={activeView?.defaultExpanded ?? settings.defaultGroupsExpanded}>
                    ...
                    {#each child.items as item (item.id)}
                      <article class="work-row">
                        ...
                        <div class="actions">
                          {#if item.active}<button class="primary" type="button" on:click={() => act(item, "add-today", { date })}>+ 今日</button>{/if}
                          <details><summary aria-label="更多操作"><MoreHorizontal size={15} /></summary><div>{#if item.active}<button type="button" on:click={() => act(item, "add-tomorrow", { date: offsetDate(date, 1) })}>加入明日</button>{/if}<button type="button" on:click={() => act(item, "open")}>打开</button>{#if item.kind === "task" && item.active}<button type="button" on:click={() => act(item, "complete-task")}><Check size={13} />完成</button><button type="button" on:click={() => act(item, "drop-task")}>放弃</button>{/if}{#if item.kind === "task" && item.taskState === "planned"}<button type="button" on:click={() => act(item, "return-task")}><RotateCcw size={13} />退回池中</button>{/if}{#if item.kind === "question"}<button type="button" on:click={() => act(item, item.lane === "think" ? "move-to-write" : "move-to-think")}>转为 {item.lane === "think" ? "ToWrite" : "ToThink"}</button>{/if}</div></details>
            ...
            {:else}
              {#each group.items as item (item.id)}
                <article class="work-row">
                  ...
                  <div class="actions">
                    {#if item.active}<button class="primary" type="button" on:click={() => act(item, "add-today", { date })}>+ 今日</button>{/if}
                    <details><summary aria-label="更多操作"><MoreHorizontal size={15} /></summary><div>{#if item.active}<button type="button" on:click={() => act(item, "add-tomorrow", { date: offsetDate(date, 1) })}>加入明日</button>{/if}<button type="button" on:click={() => act(item, "open")}>打开</button>{#if item.kind === "task" && item.active}<button type="button" on:click={() => act(item, "complete-task")}>完成</button><button type="button" on:click={() => act(item, "drop-task")}>放弃</button>{/if}{#if item.kind === "question"}<button type="button" on:click={() => act(item, item.lane === "think" ? "move-to-write" : "move-to-think")}>转为 {item.lane === "think" ? "ToWrite" : "ToThink"}</button>{/if}</div></details>
                    {#if item.kind === "note" && workflowStages.length}<select aria-label="修改 Workflow 阶段" value={item.stageId ?? ""} on:change={(event) => selectStage(item, event)}><option value="" disabled>阶段</option>{#each workflowStages as option}<option value={option.id}>{option.label}</option>{/each}</select>{/if}
                    {#if item.kind === "question" && questionStatuses.length}<select aria-label="修改问题状态" value={item.questionStatus ?? ""} on:change={(event) => selectQuestionStatus(item, event)}>{#each questionStatuses as option}<option value={option.id}>{option.label}</option>{/each}</select>{/if}
                  </div>
                </article>
