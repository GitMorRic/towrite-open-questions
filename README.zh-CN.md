...
如果你把同样的 checkbox 项目树写在日记后面的自定义标题下，ToWrite 也会识别整棵树，并在迁移时保留“项目 → 链接/子任务”的 Markdown 层级。只有孤立 checkbox 无法表达它是不是计划；要接管这类自定义区段，可在标题下写一次不可见的 Obsidian 注释：
...
不需要给每篇日记或每个任务插入标记。日记身份仍由 Daily Notes 的目录和日期格式决定；这个标记只用于明确接管一个自定义区段，避免把随记里的普通 checkbox 误识别成今日任务。
